...
func generateInsertQuery(model interface{}) string {
	...
	var colomns, placeholders string
	for i := 0; i < modelType.NumField(); i++ {
		dbTag := modelType.Field(i).Tag.Get("db")
		fmt.Println("dbTag:", dbTag)
		dbTag = strings.TrimSuffix(dbTag, ",omitempty")
		if dbTag != "" && dbTag != "id" { // skip ID field if it's auto increment
			if colomns != "" {
				colomns += ", "
				placeholders += ", "
			}
	...
	return fmt.Sprintf("INSERT INTO teachers (%s) VALUES (%s)", colomns, placeholders)
}
...
func PatchTeachers(updates []map[string]interface{}) error {
	...
	for _, update := range updates {
		...
		err = tx.QueryRow("SELECT id, first_name, last_name, email, class, subject FROM teachers WHERE id = ?", id).Scan(&teacherFromDb.ID, &teacherFromDb.FirstName, &teacherFromDb.LastName, &teacherFromDb.Email, &teacherFromDb.Class, &teacherFromDb.Subject)
		if err != nil {
			tx.Rollback()
			if err == sql.ErrNoRows {
