...
func PatchExecs(updates []map[string]interface{}) error {
	...
	for _, update := range updates {
		...
		err = tx.QueryRow("SELECT id, first_name, last_name, email, username FROM execs WHERE id = ?", id).Scan(&execFromDb.ID, &execFromDb.FirstName, &execFromDb.LastName, &execFromDb.Email, &execFromDb.Username)
		if err != nil {
			tx.Rollback()
			if err == sql.ErrNoRows {
...
func ForgotPasswordDbHandler(emailId string) error {
	...
	m := mail.NewMessage()
	m.SetHeader("From", "schooladmin@shool.com") // Replace with your own sender email
